...
{
	lenght = 0;
	fileOfUser.open("User.txt", ios::in);
...
bool loginAdmin(string user[], string& username, string& password,int lenght)
{
	SetConsoleOutputCP(65001);
	...
	do
	{
		...
		if (countFailLogin == 5)
		{
			...
			{
				cout << endl;
				cout << setw(15) << left << "" << "Bạn Đã Đăng Nhập Thất Bại Quá Nhiều! Bạn Không Thể Đăng Nhập Nữa. Bạn Sẽ Là Khách Hàng!\n";
				Sleep(5000);
				return false;
			}
			for (int i = 0; i < 15; i++)
...
int main()
{
	...
	int lenght,option;
	char checkContinue;
	char checkContinueLogin;
	menu();
	cout << setw(10) << left << "" << "Lựa Chọn Của Bạn: "; cin >> option; cin.ignore();
	system("CLS");
	switch (option)
	{
	case 1:
		...
		{
			system("CLS");
			formRegister(fileUser, username, password);
			loading();
			cout << endl << endl;
			cout << setw(20) << left << "" << "Bạn Đã Đăng Ký Thành Công!\n\n";
			cout << setw(15) << left << "" << "Bạn Muốn Tiếp Tục Đăng Ký? Y/N ---------> "; cin >> checkContinue; cin.ignore();
		} while (checkContinue == 'Y' || checkContinue == 'y');
		system("CLS");
		cout << endl;
		cout << setw(15) << left << "" << "Bạn Muốn Đăng Nhập? Y/N ---------> "; cin >> checkContinueLogin; cin.ignore();
		if (checkContinueLogin == 'y' || checkContinueLogin == 'Y')
		{
			goto login;
		}
		break;
login:	case 2:
		readFile(fileUser, user, lenght);
		if (loginAdmin(user, username, password, lenght))
		{
			//system("CLS");
			loading();
			cout << endl << endl;
			cout << setw(25) << left << "" << "Bạn Đã Đăng Nhập Thành Công!\n\n";
			Sleep(1000);
			cout << endl;
		}
		break;
